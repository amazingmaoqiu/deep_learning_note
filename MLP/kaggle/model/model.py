...
class MLP(object):
    def __init__(self):
        ...
        self.input = tf.placeholder(
            shape=[None, (2*self.padding+1)*40], dtype=tf.float32, name='inputs')
        self.label = tf.placeholder(
            shape=[None, 138], dtype=tf.float32, name='labels')
    ...
    def perceptron_layer(self, idx_layer, input, activation='relu'):
        with tf.variable_scope("pl" + str(idx_layer)) as scope:
            ...
            weights = tf.get_variable(shape=[input.shape[1], self.layer[idx_layer-1]],
                                      dtype=tf.float32, initializer=tf.truncated_normal_initializer(), name='weights')
            bias = tf.get_variable(
                shape=[self.layer[idx_layer - 1]], dtype=tf.float32,  name='bias')
    ...
    def build_model(self):
        ...
        self.net = slim.fully_connected(
            self.net, self.layer[-1], activation_fn=None)
        self.sess = tf.Session()
        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=self.net, labels=tf.cast(self.label, dtype=tf.float32)))
    ...
    def load_data(self, mode):
        if(mode == 'train'):
            ...
            label = np.load('../data/train_padding_labels.npy',
                            encoding='latin1')
            idx_table = np.load(
                '../data/train_padding_idx.npy', encoding='latin1')
        elif(mode == 'test'):
            data = np.load('../data/validation.npy', encoding='latin1')
            label = np.load('../data/validation_labels.npy', encoding='latin1')
            idx_table = np.load(
                '../data/validation_table.npy', encoding='latin1')
        elif(mode == 'validation'):
            data = np.load('../data/validation.npy', encoding='latin1')
            label = np.load('../data/validation_labels.npy', encoding='latin1')
            idx_table = np.load(
                '../data/validation_idx.npy', encoding='latin1')
        else:
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            avg_loss = 0
            shuffle = np.array(random.sample(
                range(idx_table.shape[0]), idx_table.shape[0]))
            correct_prediction = tf.equal(
                tf.argmax(self.net, axis=1), tf.argmax(self.label, axis=1))
            correct_batch = tf.reduce_sum(
            ...
            while(start < shuffle.shape[0]):
                ...
                batch_data = np.zeros(
                    [batch_idx.shape[0], 40*(2*self.padding+1)])
                for idx in range(batch_idx.shape[0]):
                    batch_data[idx, :] = data[idx_table[batch_idx[idx]] -
                                              self.padding:idx_table[batch_idx[idx]]+self.padding+1].reshape(1, -1)
                batch_label = label[batch_idx]
                batch_label = self.one_hot(batch_label)
                _, cost, correct = self.sess.run((self.optimizer, self.loss, correct_batch), feed_dict={
            ...
            while(start < val_shuffle.shape[0]):
                ...
                batch_data = np.zeros(
                    [batch_idx.shape[0], 40*(2*self.padding+1)])
                for idx in range(batch_idx.shape[0]):
                    batch_data[idx, :] = val_data[val_table[batch_idx[idx]] -
                                                  self.padding:val_table[batch_idx[idx]]+self.padding+1].reshape(1, -1)
                batch_label = val_label[batch_idx]
                batch_label = self.one_hot(batch_label)
                corr = self.sess.run(correct_batch, feed_dict={
                                     self.input: batch_data, self.label: batch_label})
                pre_cor += corr
                start += self.batch_size
            print("epoch %04d : loss = %.9f, accuracy = %.9f, val_acc = %.9f" % (
                epoch, avg_loss, total_correct / idx_table.shape[0], pre_cor / val_label.shape[0]))
    ...
    def predict(self):
        ...
        while(start < table.shape[0]):
            ...
            for idx in range(batch_idx.shape[0]):
                batch_data[idx, :] = data[batch_idx[idx] -
                                          self.padding:batch_idx[idx]+self.padding+1].reshape(1, -1)
            ans[ans_idx] = self.sess.run(prediction, feed_dict={
    ...
    def test(self):
        ...
        while(start < idx_table.shape[0]):
            ...
            for idx in range(batch_idx.shape[0]):
                batch_data[idx, :] = data[batch_idx[idx] -
                                          self.padding:batch_idx[idx]+self.padding+1].reshape(1, -1)
            total_correct += self.sess.run(correct_batch, feed_dict={
                                           self.input: batch_data, self.label: batch_label})
            start += self.batch_size
